...
export class AuthService {
  ...
  async sign_in(username: string, password: string, ctx: CtxType): Promise<AuthModel> {
    ...
    if (!user.verified) {
      throw new GraphQLError(ctx.i18n.t('auth.exception.forbidden_not_verified'), {
        extensions: { code: 'WARNING' },
      });
    }
  ...
  async register(user_input_data: UserInputModel, ctx: CtxType): Promise<UserModel | null> {
    user_input_data.password = await this.passwordService.hash(user_input_data.password);
    return this.prisma.user.create({
      select: { id: true, username: true, name: true, verified: true, challenge: true },
      data: {
        ...user_input_data,
        server_id: ctx.server_id
      }
    }).then(({ challenge, ...user }) => {
      this.emailService.send_verify(user.username, user.name, this.emailService.generate_verify_url(challenge))
      return user
    })
      .catch((e) => {
      if (e.code === "P2002") {
        throw new GraphQLError(ctx.i18n.t("user.exception.conflict_username"), {
          extensions: { code: "WARNING" }
        });
      }
  ...
  async reset_password(user_pw_reset_input_data: UserPwResetInputModel, ctx: CtxType): Promise<UserModel | null> {
    const user = await this.prisma.user.findUnique({
      where: { challenge: user_pw_reset_input_data.challenge }
    });
    if (!user || !user.pw_reset) {
      throw new GraphQLError(ctx.i18n.t("auth.exception.not_found_password_reset"), {
        extensions: { code: "WARNING" }
      });
    }
    ...
    return this.prisma.user.update({
      where: { id: user.id },
      data: {
        password: await this.passwordService.hash(user_pw_reset_input_data.password),
        pw_reset: false
      }
    });
  }

  async reset_password_request(username: string, ctx: CtxType): Promise<boolean> {
    const user = await this.prisma.user.findUnique({
      where: { username: username }
    });
    if (!user) {
    ...
    await this.emailService.send_password_reset(user.username, user.name, pw_reset_url)
    return true;
  }
}
